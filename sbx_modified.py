...
##Modified by AL april 2024, put into suite2p io folder and rename as sbx.py
...
def sbx_to_binary(ops, ndeadcols=-1, ndeadrows=0):
    ...
    for ifile, sbxfname in enumerate(sbxlist):
        ...
        for ichunk, onset in enumerate(iblocks[:-1]):
            ...
            im = np.array(f[onset:offset, :, :, ndeadrows:-ndeadrows, ndeadcols:-ndeadcols]) // 2
            # changed the indexing of this part to remove n number of dead cols and dead rows from both start and end
